...
export class SportEquipmentService {
  ...
  /**
   * Get all pending ownership requests (waiting status)
   * Also fetches sport equipment names from external API
   */
  async getPendingOwnershipRequests(): Promise<{
    requests: OwnerSportEquipment[]
    namesMap: Map<string, string>
  }> {
    const requests = await OwnerSportEquipment.query()
      .where('status', 'waiting')
      .preload('owner')
      .orderBy('createdAt', 'desc')

    // Get unique equipment IDs
    const equipmentIds = [...new Set(requests.map((r) => r.sportEquipmentId))]

    // Fetch names for each equipment
    const namesMap = new Map<string, string>()
    await Promise.all(
      equipmentIds.map(async (id) => {
        try {
          const equipment = await this.getSportEquipmentById(id)
          if (equipment) {
            namesMap.set(id, equipment.equip_nom)
          }
        } catch {
          // If we can't fetch the name, we'll just use the ID
        }
      })
    )

    return { requests, namesMap }
  }
}
